...
const moveTemplateToUserCurrentWorkingDirectory = (name) => (
  fs.moveSync(path.resolve(buildDir, 'lib/template'), path.resolve(CWD, name), (err) => {
    if (err) {
      console.error(err);
    } else {
...
const prepBuild = () => {
  if(!process.argv.slice(2)[0]){
    // if no args
    return promptUserForProjectName().then((res) => moveTemplateToUserCurrentWorkingDirectory(res.name))
  } else if (process.argv.slice(2)[1]) {
    // if to many args
    console.log()
    console.log('lg-coffe setup')
    console.log()
    console.log(`To Many Arguments! Remove "${process.argv.slice(2)[1]}" and try your command again.`)
    console.log()
    ...
    console.log('lg-coffe setup')
    console.log()
    console.log(`building project in ${ path.resolve(CWD, process.argv.slice(2)[0]) }`)
